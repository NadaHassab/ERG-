"""Ultimate Fusion: MultiDomain + CWT + Advanced SP

Combines ALL signal processing features into one model:
- Time domain: signal (128,) via CNN
- OT domain: signed OT (135,)
- Spectral domain: FFT features (10,)
- VMD domain: VMD features (80,)
- Physical features: (8,)
- CWT scalogram (16 scales x 128)
- Advanced SP: Hjorth, TKEO, entropy, fractal, EMD, Welch PSD (32 dims)
"""
...
from scripts.run_advanced_sp import (
    compute_all_features, FEAT_DIM as ADVSP_DIM,
    bootstrap_auroc,
)
...
OUT_DIR = Path("artifacts/results/ultimate_fusion_v1")
N_SCALES = 16


class UltimateFusion(nn.Module):
    """7-branch fusion: signal + OT + spectral + VMD + physical + CWT + advanced SP."""
    ...
    def __init__(self, d_model=128, n_heads=4, n_layers=2, dropout=0.1, seed=1001):
        ...
        # Branch 7: MLP on advanced SP features
        self.advsp_mlp = nn.Sequential(
            nn.Linear(ADVSP_DIM, 64), nn.LayerNorm(64), nn.GELU(),
            nn.Dropout(dropout), nn.Linear(64, d_model),
        )

        # Gated fusion (7 domains)
        self.gate = nn.Sequential(nn.Linear(d_model * 7, 7), nn.Softmax(dim=-1))
    ...
    def forward(self, signal, vmask, ot, spectral, vmd, physical, cwt, advsp, comp_mask=None):
        B, L = signal.shape[:2]
        ...
        cwt_feat = self.cwt_proj(self.cwt_cnn(cwt.reshape(B*L, N_SCALES, -1)).squeeze(-1))
        advsp_feat = self.advsp_mlp(advsp.reshape(B*L, -1))

        concat = torch.cat([sig_feat, ot_feat, spec_feat, vmd_feat, phys_feat, cwt_feat, advsp_feat], dim=-1)
...
def collate_ultimate(bags, dataset, scal_cache, advsp_cache, comp_idx_map):
    B = len(bags)
    ...
    advsp = np.zeros((B, L, ADVSP_DIM), dtype=np.float32)
    ...
    return {
        "signal": torch.as_tensor(signal), "valid_mask": torch.as_tensor(valid_mask),
        "ot": torch.as_tensor(ot), "physical": torch.as_tensor(physical),
        "spectral": torch.as_tensor(spectral), "vmd": torch.as_tensor(vmd),
        "cwt": torch.as_tensor(cwt), "advsp": torch.as_tensor(advsp),
        "comp_mask": torch.as_tensor(comp_mask), "label": torch.as_tensor(labels),
    }


def eval_auc(model, dataset, scal_cache, advsp_cache, comp_idx_map, bags, device):
    model.eval()
    y_true, y_prob = [], []
    for bag in bags:
        if bag.target_binary is None:
            continue
        batch = collate_ultimate([bag], dataset, scal_cache, advsp_cache, comp_idx_map)
        with torch.no_grad():
            logit, _ = model(
                ...
                batch["cwt"].to(device), batch["advsp"].to(device),
...
def train_model(model, train_bags, val_bags, dataset, scal_cache, advsp_cache, comp_idx_map, seed, device, lr=1e-4):
    model.to(device)
    ...
    for epoch in range(200):
        for step, idx in enumerate(sampler):
            ...
            batch = collate_ultimate(bags_batch, dataset, scal_cache, advsp_cache, comp_idx_map)
            ...
            optimizer.step()
            sched.step()
        val_auc = eval_auc(model, dataset, scal_cache, advsp_cache, comp_idx_map, val_bags, device)
        if val_auc > best_auc:
            best_auc = val_auc
...
def predict_model(model, dataset, scal_cache, advsp_cache, comp_idx_map, bags, device):
    model.eval()
    rows = []
    for bag in bags:
        if bag.target_binary is None:
...
def main():
    ...
    for task in ["LEOP", "PERG"]:
        print(f"\n{'='*60}")
        print(f"  {task} — Ultimate Fusion (7 domains)")
        print(f"{'='*60}")
        ...
        # Precompute CWT + advanced SP
        scal_cache = precompute_scalograms(bags, N_SCALES)
        advsp_cache = {}
        for bag in bags:
            for comp in bag.components:
                key = comp.global_component_id
                if key not in advsp_cache:
                    advsp_cache[key] = compute_all_features(comp.signal)
        print(f"  Precomputed {len(scal_cache)} CWT scalograms, {len(advsp_cache)} advanced SP vectors")
        ...
        for seed in SEEDS:
            ...
            for outer_fold in range(5):
                ...
                model = UltimateFusion(seed=seed)
                log = train_model(model, train_bags, test_bags, ds, scal_cache, advsp_cache, comp_idx_map, seed, DEVICE)
                pred = predict_model(model, ds, scal_cache, advsp_cache, comp_idx_map, test_bags, DEVICE)
