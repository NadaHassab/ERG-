"""Direction 4: Bidirectional SSM ERG classifier.

Uses LSTM as a practical SSM alternative — GPU-optimized,
...
class SSMERGClassifier(nn.Module):
    def __init__(self, signal_len=128, cnn_ch=64, d_model=128,
                 n_layers=2, dropout=0.1, seed=1001):
        super().__init__()
    ...
    def forward(self, signal, valid_mask):
        ...
        lstm_out, _ = self.lstm(x)  # (B, L, d_model)

        aw = self.scorer(lstm_out).squeeze(-1).masked_fill(cv == 0, float("-inf"))
        aw = F.softmax(aw, dim=-1).masked_fill(cv == 0, 0.0)
        pooled = (lstm_out * aw.unsqueeze(-1)).sum(dim=1)
