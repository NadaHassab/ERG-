"""Direction 5b: Supervised contrastive + classification ERG."""
...
class SupConERGClassifier(nn.Module):
    def __init__(self, cnn_ch=64, d_model=128, proj_dim=64, dropout=0.1, seed=1001):
        super().__init__()
        ...
        self.proj_enc = nn.Linear(cnn_ch, d_model)
        self.proj_head = nn.Sequential(nn.Linear(d_model, d_model), nn.GELU(), nn.Linear(d_model, proj_dim))
    ...
    def forward(self, signal, valid_mask):
        ...
        x = self.proj_enc(self.cnn(x).squeeze(-1)).view(B, L, -1)
...
def train_one_task(model, train_bags, val_bags, task, seed, device="cuda"):
    ...
    bce_crit = FoldWeightedBCE(positive_class_weight(labels))
    ...
    for epoch in range(200):
        for step, idx in enumerate(sampler):
            ...
            logits, z, _ = model(sig, vm)
            loss = bce_crit(logits, lb) + 0.1 * supcon_loss(z, lb.long())
...
def eval_auc(model, bags, task, device):
    ...
    for bag in bags:
        ...
        sig = torch.as_tensor(batch["signal"], dtype=torch.float32, device=device)
        vm = torch.as_tensor(batch["valid_mask"], dtype=torch.bool, device=device)
        with torch.no_grad(): logit, _, _ = model(sig, vm)
...
def main():
    ...
    OUT = Path("artifacts/results/supcon_erg_v1")
    ...
    for task in ["LEOP", "PERG"]:
        ...
        for seed in SEEDS:
            ...
            for fold in range(5):
                ...
                model = SupConERGClassifier(seed=seed)
